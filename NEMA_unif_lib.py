...
def dome_correction(imagearray):
   ...
   params = fitflux(data)
   print("A, R_0, center_x, center_y : {}".format(params))
...
def nema_data_preprocess(imagearray,resamplesize):
    ...
    print("array size: {}    resamplesize: {}".format(np.shape(imagearray), resamplesize))
    ...
    if resamplesize[0]>0 and resamplesize[1]>0:
      imagearray = block_reduce(imagearray, block_size=(int(np.shape(imagearray)[0]/resamplesize[0]),int(np.shape(imagearray)[1]/resamplesize[1])),func=np.sum)
